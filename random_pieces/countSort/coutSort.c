...
{
  for (int i = 0; i < 4000000; i++ ) sample();
...
int * fill_with_random_integers(int *a, const size_t size)
{
  for (int i = 0; i < size; i++) a[i] = get_random_int(VECTORSORTSIZE - 1, 0);
...
void sort(int values[], int n)
{
  int counter[ VECTORSORTSIZE ];

  for ( unsigned int i = 0; i < VECTORSORTSIZE; i++ ) counter[ i ] = 0;

  for ( int i = 0; i < n; i++ ) counter[ values[ i ] ]++;

  for ( int i = 0, index = 0; i < VECTORSORTSIZE; i++ ) {
    while ( counter[ i ] > 0 ) {
      values[ index++ ] = i;
      counter[ i ]--;
    }
